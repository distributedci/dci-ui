...
export function addAnsibleStars(fileName: string) {
  const maxNumberOfStars = 80;
  const spaceLength = 1;
  if (fileName.length >= maxNumberOfStars - spaceLength) {
    return `${fileName} ***`;
  }
  return `${fileName} `.padEnd(maxNumberOfStars, "*");
}
