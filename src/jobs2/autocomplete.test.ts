...
test("getOptions trim extra space if present", () => {
  ...
  expect(getOptions("name= ", availableOptions)).toEqual([
    "name=",
  ]);
});

// test("autocomplete with an empty Trie", () => {
//   const trie = new Trie();
//   expect(trie.autocomplete("s")).toEqual([]);
// });

// test("autocomplete with 2 matches", () => {
//   const trie = new Trie();
//   for (const field of ["name", "state", "status"]) {
//     trie.insert(field);
//   }
//   expect(trie.autocomplete("s")).toEqual(["state", "status"]);
// });

// test("autocomplete with an existing match", () => {
//   const trie = new Trie();
//   for (const field of ["name", "state", "status"]) {
//     trie.insert(field);
//   }
//   expect(trie.autocomplete("name s")).toEqual(["name state", "name status"]);
// });
